...
app.use('*', async (c, next) => {
  ...
  // Override for /view and /demo routes (allow embedding)
  if (c.req.path.startsWith('/view') || c.req.path.startsWith('/demo')) {
...
// Requires password verification and deletes all documents from D1 and R2
app.post('/api/admin/delete-library', async (c) => {
  ...
  try {
    ...
    // Verify password (same logic as login)
    const customPassword = await kv.get('admin_password_hash')
    let isPasswordValid = false
    
    if (customPassword) {
      const passwordData = JSON.parse(customPassword)
      isPasswordValid = await verifyPassword(password, passwordData.hash, passwordData.salt)
    }
    
    // Fallback to master password
    if (!isPasswordValid && adminPassword && password === adminPassword) {
      isPasswordValid = true
    }
    
    if (!isPasswordValid) {
      console.warn('⚠️ DELETE LIBRARY: Invalid password attempt')
      return c.json({ success: false, error: 'Invalid password' }, 401)
    }
    
    console.log('🗑️ DELETE LIBRARY: Password validated, starting deletion...')
    
    // Step 1: Get all documents from D1 to know which R2 files to delete
    const documentsResult = await db.prepare('SELECT token, r2_key FROM documents').all()
    ...
    for (const doc of documents) {
      try {
        await r2.delete(doc.r2_key as string)
        deletedFilesCount++
        
        // Log progress every 10 files
        if (deletedFilesCount % 10 === 0) {
          console.log(`  ↳ R2 deletion progress: ${deletedFilesCount}/${documents.length}`)
        }
      } catch (error) {
        console.error(`  ❌ Failed to delete R2 file: ${doc.r2_key}`, error)
        failedDeletions.push(doc.r2_key as string)
      }
    }
    
    console.log(`✅ R2 deletion complete: ${deletedFilesCount} files deleted`)
    
    if (failedDeletions.length > 0) {
      console.warn(`⚠️ ${failedDeletions.length} R2 deletions failed:`, failedDeletions)
    }
    
    // Step 3: Delete all documents from D1
    const deleteResult = await db.prepare('DELETE FROM documents').run()
    const deletedDocumentsCount = deleteResult.meta?.changes || documents.length
    
    console.log(`✅ D1 deletion complete: ${deletedDocumentsCount} documents deleted`)
    
    // Calculate duration
    const duration = Date.now() - startTime
    
    console.log(`🎉 LIBRARY DELETION COMPLETE in ${duration}ms`)
    console.log(`   - D1 documents deleted: ${deletedDocumentsCount}`)
    console.log(`   - R2 files deleted: ${deletedFilesCount}`)
    console.log(`   - Failed R2 deletions: ${failedDeletions.length}`)
    ...
    return c.json({ 
      success: false, 
      error: 'Failed to delete library: ' + (error as Error).message 
    }, 500)
  }
})
...
app.post('/api/admin/upload', async (c) => {
  try {
    ...
    return c.json({ 
      success: true, 
      token,
      // Use storybook-mobile-reader for mobile-optimized viewer
      shareUrl: `https://storybook-mobile-reader.pages.dev/view?doc=${token}`
    })
  } catch (error) {
...
app.get('/api/documents/:token', async (c) => {
  try {
    ...
    // 🔒 DEMO DOCUMENT: Hardcoded sanctuarized demo file
    // This document is protected and served from static files
    // Token: 21edaf29-7fc6-4478-9e00-e63f8afccfe5 (DO NOT CHANGE)
    if (token === '21edaf29-7fc6-4478-9e00-e63f8afccfe5') {
      console.log('📚 [DEMO] Serving sanctuarized demo document')
      const demoFile = await c.env.ASSETS.fetch(new Request('https://fake-host/demo/bitcoin.pdf'))
      
      if (!demoFile.ok) {
        console.error('❌ [DEMO] Failed to fetch demo file from static assets')
        return c.json({ success: false, error: 'Demo document not found' }, 404)
      }
      
      return new Response(demoFile.body, {
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': 'inline; filename="L-Or-et-le-BITCOIN-une-fable.pdf"; filename*=UTF-8\'\'L-Or-et-le-BITCOIN-une-fable.pdf',
...
app.get('/', (c) => {
  return c.html(`
    ...
    <head>
        ...
        <title>Training Storybook INSURACTIO - Solution de Visualisation Interactive de Documents PDF</title>
        <meta name="description" content="Training Storybook INSURACTIO : solution professionnelle open source pour visualiser, gérer et partager vos contenus de formation avec effet de page tournée et bibliothèque intelligente powered by Gemini 2.5 Flash.">
        
        <!-- Open Graph / LinkedIn / Facebook -->
        <meta property="og:type" content="website">
        <meta property="og:url" content="https://storybook.insuractio.com">
        <meta property="og:title" content="Training Storybook INSURACTIO - Visualisation Interactive de Documents PDF">
        <meta property="og:description" content="Solution professionnelle open source pour la formation en bancassurance. Viewer interactif avec effet page tournée, QR codes, et analyse IA automatique des contenus pédagogiques.">
        <meta property="og:site_name" content="Training Storybook INSURACTIO">
        <meta property="og:locale" content="fr_FR">
        <meta property="og:image" content="https://storybook.insuractio.com/static/og-image.png">
        <meta property="og:image:width" content="1200">
        <meta property="og:image:height" content="630">
        <meta property="og:image:type" content="image/png">
        <meta property="og:image:alt" content="Training Storybook INSURACTIO - Solution de visualisation interactive de documents PDF">
        <meta property="article:author" content="Florent Siegenthaler - INSURACTIO">
        <meta property="article:published_time" content="2025-11-14T00:00:00Z">
        
        <!-- Twitter Card -->
        <meta name="twitter:card" content="summary_large_image">
        <meta name="twitter:title" content="Training Storybook INSURACTIO">
        <meta name="twitter:description" content="Solution open source pour visualiser et partager des PDF interactifs avec effet de page tournée">
        <meta name="twitter:image" content="https://storybook.insuractio.com/static/og-image.png">
        <meta name="twitter:image:alt" content="Training Storybook INSURACTIO">
        
        <!-- Keywords -->
        <meta name="keywords" content="formation, bancassurance, PDF, viewer, page flip, QR code, IA, Gemini, open source, INSURACTIO, storybook, pédagogie">
        <meta name="author" content="Florent Siegenthaler - INSURACTIO">
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="demo-section">
                ...
                <iframe 
                    src="https://storybook-mobile-reader.pages.dev/view?doc=21edaf29-7fc6-4478-9e00-e63f8afccfe5" 
                    class="demo-iframe"
                    allowfullscreen 
                    title="Training Storybook Mobile viewer demonstration">
                </iframe>
                <p style="text-align: center; opacity: 0.6; margin-top: 1rem; font-size: 0.9rem;">
...
app.get('/admin', (c) => {
  return c.html(`
    ...
    <head>
        ...
        <meta name="description" content="Interface d'administration Training Storybook INSURACTIO. Gérez vos documents de formation, générez des QR codes et analysez vos contenus pédagogiques avec l'IA.">
        
        <!-- Open Graph / LinkedIn -->
        <meta property="og:type" content="website">
        <meta property="og:url" content="https://storybook.insuractio.com/admin">
        <meta property="og:title" content="Administration - Training Storybook INSURACTIO">
        <meta property="og:description" content="Interface d'administration pour gérer et partager vos contenus de formation avec QR codes et analyse IA. Solution professionnelle pour la bancassurance.">
        <meta property="og:site_name" content="Training Storybook INSURACTIO">
        <meta property="og:locale" content="fr_FR">
        <meta property="og:image" content="https://storybook.insuractio.com/static/og-image.png">
        <meta property="og:image:width" content="1200">
        <meta property="og:image:height" content="630">
        <meta property="og:image:type" content="image/png">
        <meta property="og:image:alt" content="Training Storybook INSURACTIO - Interface d'administration">
        <meta property="article:author" content="Florent Siegenthaler - INSURACTIO">
        <meta property="article:published_time" content="2025-11-14T00:00:00Z">
        
        <!-- Twitter Card -->
        <meta name="twitter:card" content="summary">
        <meta name="twitter:title" content="Administration - Training Storybook">
        <meta name="twitter:description" content="Gérez vos documents de formation professionnels avec Training Storybook">
        <meta name="twitter:image" content="https://storybook.insuractio.com/static/og-image.png">
        <meta name="twitter:image:alt" content="Training Storybook INSURACTIO Admin">
...
// Demo route - Public access to demonstration document (no authentication required)
app.get('/demo', async (c) => {
  const DEMO_TOKEN = '21edaf29-7fc6-4478-9e00-e63f8afccfe5'
  
  // Always show desktop viewer for demo (no mobile redirect)
  return c.html(`
    <!DOCTYPE html>
    <html lang="fr">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Démonstration - Training Storybook</title>
        <link href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css" rel="stylesheet">
    ...
    <body>
        <div id="loading">
            ...
            <p id="loading-text">Chargement du document de démonstration...</p>
        </div>
            ...
            <div class="viewer-header">
                <div class="viewer-controls">
                    ...
                    <div style="display: flex; gap: 0.5rem;">
                        ...
                        <button class="control-btn" id="page-mode-btn" title="Basculer affichage 1/2 pages (M)">
        ...
        <script>
            // Override URL params for demo - set demo token in query string for viewer.js
            if (!window.location.search.includes('doc=')) {
                const demoToken = '${DEMO_TOKEN}';
                window.history.replaceState({}, '', '?doc=' + demoToken);
            }
        </script>
...
app.get('/view', async (c) => {
  ...
  // Security: Verify document exists before allowing access
  try {
    const db = c.env.DB
    const doc = await db.prepare('SELECT token FROM documents WHERE token = ?')
      .bind(docToken)
      .first()
    ...
    if (!doc) {
      return c.html(`
        ...
        <body class="bg-gray-900 text-white flex items-center justify-center min-h-screen">
            <div class="text-center">
                <h1 class="text-3xl font-bold mb-4">Document non trouvé</h1>
                <p class="text-gray-400">Ce document n'existe pas ou a été supprimé.</p>
            </div>
        </body>
        </html>
      `)
  ...
  // Smart routing: Detect mobile and redirect to storybook-mobile-reader
  const userAgent = c.req.header('user-agent') || ''
  const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(userAgent)
  
  if (isMobile) {
    // Redirect mobile users to Flutter app with standalone mode (custom domain with SSL)
    return c.redirect(`https://reader.insuractio.com/view?doc=${docToken}&standalone=true`)
  }
  
  // Desktop: Show training-storybook viewer with StPageFlip
  return c.html(`
    <!DOCTYPE html>
    <html lang="fr">
    <head>
...
async function analyzeWithGemini(text: string, imageBase64: string | null, apiKey: string, isScanned: boolean = false, totalPages: number | null = null, sampledPages: number | null = null) {
  ...
  const parts: any[] = [
    {
      text: `Tu es un assistant spécialisé dans l'analyse de documents professionnels (formation, assurance, banque, conseil).

INSTRUCTIONS D'ANALYSE :
1. LIS ATTENTIVEMENT TOUTES LES PAGES FOURNIES (première + dernière + intermédiaires)
2. LA DERNIÈRE PAGE contient souvent des informations clés : conclusions, contacts, auteurs, dates, mentions légales
3. IDENTIFIE le contexte métier : formation, documentation, guide, narration, étude de cas
4. DÉTECTE les personnages, dialogues, scénarios → indique "Formation narrative" ou "Cas pratique"
5. EXTRAIS les informations factuelles : sujet, thème, secteur d'activité

CONTEXTE DU DOCUMENT :${contextInfo}

FORMAT DE RÉPONSE (JSON strict uniquement, sans commentaire) :
{
  "filename": "nom-descriptif-du-sujet-principal.pdf",
  "description": "Description factuelle incluant le type de document et le contexte métier",
  "tags": ["tag1", "tag2", "tag3"],
  "folder": "Catégorie/Sous-catégorie"
}

RÈGLES STRICTES :
- filename : kebab-case, décrit le SUJET RÉEL (pas "art-de-l-ecoute" si c'est du conseil bancaire), max 50 caractères, extension .pdf
- description : DOIT mentionner le type (guide, formation narrative, cas pratique, etc.) + le secteur (bancassurance, conseil, formation) + sujet principal${totalPages ? `, mentionne "${totalPages} pages"` : ''}
  Exemple : "Formation narrative de 21 pages sur le devoir de conseil en bancassurance à travers le personnage de Claire"
- tags : 3-5 mots-clés précis reflétant le contenu (ex: si dialogue sur conseil bancaire → ["conseil", "bancassurance", "devoir-conseil", "formation", "vente"])
- folder : hiérarchie métier logique (ex: "Formation/Bancassurance/Devoir-Conseil" pas "Formation/Communication")

EXEMPLES DE BON ANALYSE :
- Document avec dialogue "Voici la fiche standardisée... conseiller, pas seulement vendre" → folder: "Formation/Bancassurance", tags: ["conseil", "vente-conseil", "bancassurance"]
- Document technique assurance → folder: "Documentation/Assurance", tags spécifiques au produit
- Cas pratique commercial → folder: "Formation/Cas-Pratiques", description mentionne "cas pratique"
