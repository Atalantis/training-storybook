...
async function compressPDF(file, progressCallback = null) {
    ...
    try {
        ...
        for (let pageNum = 1; pageNum <= pageCount; pageNum++) {
            ...
            // Convert canvas to JPEG with quality 0.95 (high quality for better readability)
        ...
        <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800 p-4">
            <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-lg w-full text-center">
                <div class="mb-6">
                    ...
                    <i class="fas fa-times text-3xl text-red-500 ml-2"></i>
                </div>
                
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800 mb-4">
                    Interface Desktop Requise
                </h1>
                
                <p class="text-gray-600 mb-6 text-left">
                    L'interface d'administration <strong>Training Storybook Admin</strong> nécessite un écran plus large pour une utilisation optimale.
                </p>
                
                <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6 text-left">
                    <p class="text-sm text-blue-800">
                        <i class="fas fa-info-circle mr-2"></i>
                        <strong>Recommandation :</strong> Utilisez un ordinateur de bureau, un ordinateur portable ou une tablette en mode paysage pour accéder à cette interface.
                    </p>
                </div>
                
                <div class="space-y-4">
                    <h3 class="font-semibold text-gray-800 text-left mb-2">Options disponibles :</h3>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg text-left">
                        ...
                        <p class="text-sm text-gray-600 mb-3">
                            Pour consulter des documents de formation sur mobile, utilisez le lecteur dédié :
                        </p>
                        <a 
                            href="https://reader.insuractio.com" 
                            class="inline-block bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg transition w-full text-center"
                        >
                            <i class="fas fa-external-link-alt mr-2"></i>
                            Ouvrir le Lecteur Mobile
                        </a>
                    </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg text-left">
                        ...
                        <p class="text-sm text-gray-600">
                            Pour gérer les documents, statistiques et configurations, revenez sur un appareil desktop.
                        </p>
                    </div>
                </div>
                ...
                <div class="mt-8 pt-6 border-t border-gray-200">
                    <p class="text-xs text-gray-500">
                        ...
                        Besoin d'aide ? Contactez le support INSURACTIO
                    </p>
                </div>
            </div>
        </div>
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (isMobile) {
        showMobileWarning();
        return; // Stop admin panel loading on mobile
    }
...
function showLibraryContent() {
    ...
    content.innerHTML = `
            ...
            <form id="upload-form" class="space-y-4">
                ...
                <!-- Compression Option -->
                <div class="bg-gray-700 p-4 rounded-lg border-2 border-amber-500">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input 
                            type="checkbox" 
                            id="upload-skip-compression" 
                            class="w-5 h-5 cursor-pointer"
                        />
                        <div class="flex-1">
                            <p class="text-white text-sm font-semibold flex items-center gap-2">
                                <i class="fas fa-bolt text-amber-400"></i>
                                Ignorer la compression automatique
                            </p>
                            <p class="text-gray-400 text-xs mt-1">
                                Uploader le fichier original tel quel sans compression (> 10 MB). 
                                Utile pour préserver la qualité maximale ou si le PDF est déjà optimisé.
                            </p>
                            <p class="text-amber-300 text-xs mt-1">
                                ⚠️ Fichiers volumineux = temps d'upload plus long
                            </p>
                        </div>
                    </label>
                </div>
    ...
    content.innerHTML = `
        <div class="bg-gray-800 rounded-xl shadow-xl p-6 border border-gray-700">
            <h2 class="text-xl font-bold text-white mb-4 flex items-center gap-2">
                ...
                Convertisseur & Optimiseur PDF
            </h2>
            
            <p class="text-gray-300 mb-6">
                Préparez vos PDFs pour la bibliothèque : optimisez les pages doubles (livrets/magazines) en pages simples lisibles, 
                supprimez les pages inutiles, et ajustez la qualité pour un affichage optimal.
            </p>
                    ...
                    <div class="space-y-4">
                            ...
                            <div class="space-y-3">
                                <label class="flex items-start gap-3 bg-gray-600 p-3 rounded cursor-pointer hover:bg-gray-550 transition">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-gray-400 text-xs mb-2">Chaque page PDF = 1 page (format portrait ou paysage standard)</p>
                                        
                                        <!-- Sub-option: Skip first page -->
                                        <div id="single-suboptions" class="mt-2 pl-6 border-l-2 border-blue-400">
                                            <label class="flex items-start gap-2 cursor-pointer">
                                                <input type="checkbox" id="skip-first-page" class="mt-1" />
                                                <div>
                                                    <p class="text-white text-sm">🗑️ Ignorer la première page</p>
                                                    <p class="text-gray-400 text-xs">Utile si la première page est blanche ou une page de garde</p>
                                                </div>
                                            </label>
                                        </div>
                                    </div>
                        ...
                        <div class="bg-gray-700 p-4 rounded-lg">
                            ...
                            <select id="quality-select" class="w-full px-4 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white focus:ring-2 focus:ring-blue-500">
                                ...
                                <option value="0.85">Qualité moyenne</option>
                                <option value="0.95" selected>Haute qualité (recommandé)</option>
                                <option value="1.0">Qualité maximale (fichier très lourd)</option>
                            </select>
                        </div>
                    </div>
                </div>
    ...
    if (converterTags.length === 0) {
        ...
        container.innerHTML = converterTags.map(tag => {
            ...
            <span class="inline-flex items-center gap-1 bg-blue-600 text-white px-3 py-1 rounded text-sm">
                ${tag}
                <button onclick="removeConverterTag('${escapedTag}')" class="hover:text-red-300" type="button">
                    <i class="fas fa-times"></i>
                </button>
            </span>
        `;}).join('');
...
function updateUploadTagsDisplay() {
    ...
    if (uploadTags.length === 0) {
        ...
        container.innerHTML = uploadTags.map(tag => {
            ...
            <span class="inline-flex items-center gap-1 bg-blue-600 text-white px-3 py-1 rounded text-sm">
                ${tag}
                <button onclick="removeUploadTag('${escapedTag}')" class="hover:text-red-300" type="button">
                    <i class="fas fa-times"></i>
                </button>
            </span>
        `;}).join('');
...
async function handleUpload(e) {
    ...
    try {
        if (isBatch) {
            ...
            const skipCompression = document.getElementById('upload-skip-compression')?.checked || false;
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                // STEP 1: COMPRESSION (if needed and not skipped)
                if (!skipCompression && shouldCompressPDF(file)) {
                ...
                } else if (skipCompression && shouldCompressPDF(file)) {
                    detailsDiv.innerHTML += `<div class="text-sm text-amber-300">⚡ ${file.name}: ${formatBytes(file.size)} (compression ignorée)</div>`;
                } else {
            ...
            if (!skipCompression && shouldCompressPDF(file)) {
                ...
                detailsDiv.innerHTML = `<div class="text-amber-300">⚡ Compression ignorée (${formatBytes(file.size)} - fichier original préservé)</div>`;
            }
...
function populateClientFilter() {
    ...
    // Extract unique client tags
    const clientsSet = new Set();
    allDocuments.forEach(doc => {
        const clientTags = parseTags(doc.client_tags);
        clientTags.forEach(client => clientsSet.add(client));
    });
    
    const clients = [...clientsSet].sort();
...
function filterDocuments() {
    ...
    if (!searchInput || !folderFilter || !tagFilter || !clientFilter) return;
    ...
    const filtered = allDocuments.filter(doc => {
        ...
        if (selectedClient) {
            const clientTags = parseTags(doc.client_tags);
            if (!clientTags.includes(selectedClient)) {
                return false;
            }
        }
...
function renderDocument(doc) {
    ...
    // Share link: Use custom domain for smart routing (mobile → reader.insuractio.com, desktop → StPageFlip)
    const shareUrl = `https://storybook.insuractio.com/view?doc=${doc.token}`;
    ...
    modal.innerHTML = `
        <div class="bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full border border-gray-700">
            <div class="p-6">
                ...
                <!-- AI Suggestions Zone (hidden by default, shown during/after analysis) -->
                <div id="ai-suggestions-zone" class="mt-4 hidden">
                    <!-- Content will be injected dynamically during AI analysis -->
                </div>
...
function showShareModal(token, filename) {
    // QR Code / Share link: Use custom domain for smart routing
    // Mobile devices → redirected to reader.insuractio.com with &standalone=true
    // Desktop devices → StPageFlip viewer
    const mobileShareUrl = `https://storybook.insuractio.com/view?doc=${token}`;
    // Desktop iframe: Custom domain (StPageFlip viewer, embedded mode)
    const desktopEmbedUrl = `https://storybook.insuractio.com/view?doc=${token}`;
        ...
        <div class="space-y-6">
            ...
            <div class="bg-gray-700 rounded-lg p-4 border border-gray-600">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <button 
                        onclick="copyToClipboard('${mobileShareUrl}')"
                ...
                <p class="text-gray-400 text-xs mt-2">
                    ...
                    Redirection intelligente : mobile → app Flutter / desktop → viewer StPageFlip
                </p>
            </div>
            ...
            <div class="bg-gray-700 rounded-lg p-4 border border-gray-600">
                ...
                <textarea 
                    ...
                    onclick="this.select()"
                ><iframe src="${desktopEmbedUrl}" width="100%" height="800" frameborder="0" allowfullscreen style="border: 1px solid #ccc; border-radius: 8px;"></iframe></textarea>
                <p class="text-gray-400 text-xs mt-2">
                ...
                <div class="mt-4 space-y-2">
                    <label class="flex items-center gap-2 text-gray-300 text-sm">
                        <input type="checkbox" id="iframe-fullscreen-${token}" checked onchange="updateIframeCode('${token}', '${desktopEmbedUrl}')">
                    ...
                    <div class="flex gap-4">
                        <div class="flex-1">
                            ...
                            <input 
                                ...
                                value="100%" 
                                onchange="updateIframeCode('${token}', '${desktopEmbedUrl}')"
            ...
            <div class="bg-gray-700 rounded-lg p-4 border border-gray-600">
                ...
                <p class="text-gray-400 text-xs mt-2">
                    <i class="fas fa-info-circle mr-1"></i>
                    Aperçu desktop (StPageFlip). Sur mobile, le lien redirige vers l'app Flutter.
                </p>
            </div>
        </div>
    `;
...
function showSecurityContent() {
    ...
    content.innerHTML = `
        ...
        <div class="bg-gray-800 rounded-xl shadow-xl p-6 border border-gray-700 max-w-3xl mx-auto mt-6">
            <div class="flex items-center gap-3 mb-6">
                <i class="fas fa-database text-red-400 text-2xl"></i>
                <h2 class="text-2xl font-bold text-white">Gestion de la Bibliothèque</h2>
            </div>
            ...
            <div class="bg-red-900 bg-opacity-30 border border-red-700 rounded-lg p-4 mb-6">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        <p class="text-red-300 font-semibold mb-2">⚠️ Zone Dangereuse</p>
                        <p class="text-red-200 text-sm">
                            Les actions ci-dessous sont <strong>irréversibles</strong>. 
                            Toutes les données supprimées seront <strong>définitivement perdues</strong>, 
                            y compris dans les sauvegardes Cloudflare.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-700 rounded-lg p-6 border-2 border-red-600">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex-1">
                        ...
                        <p class="text-gray-300 text-sm mb-3">
                            Supprime <strong>définitivement</strong> tous les documents, métadonnées, fichiers R2, et données D1.
                        </p>
                        <div class="bg-gray-800 rounded p-3 text-sm text-gray-300 mb-3">
                            <p class="font-semibold text-white mb-1">Ce qui sera supprimé :</p>
                            <ul class="list-disc list-inside space-y-1">
                                <li>Tous les documents de la base D1 (métadonnées, descriptions, tags, dossiers)</li>
                                <li>Tous les fichiers PDF du bucket R2 (stockage permanent)</li>
                                <li>Toutes les statistiques de vues</li>
                            </ul>
                        </div>
                        <div class="bg-red-900 bg-opacity-50 rounded p-3 text-sm mb-4">
                            <p class="text-red-300 font-semibold mb-1">
                                <i class="fas fa-ban mr-1"></i>
                                Restauration impossible :
                            </p>
                            <ul class="list-disc list-inside space-y-1 text-red-200">
                                <li>❌ Aucune sauvegarde automatique Cloudflare</li>
                                <li>❌ Impossible de récupérer depuis l'interface Cloudflare</li>
                                <li>❌ Les données sont écrasées de manière sécurisée</li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <button 
                    onclick="showDeleteLibraryConfirmation()" 
                    class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 px-6 rounded-lg transition flex items-center justify-center gap-2 border-2 border-red-500"
                >
            ...
            <div class="mt-4 bg-blue-900 bg-opacity-30 border border-blue-700 rounded-lg p-4">
                <p class="text-sm text-blue-300">
                    <i class="fas fa-info-circle mr-2"></i>
                    <strong>Recommandation :</strong> Avant toute suppression, exportez vos données manuellement 
                    via Wrangler CLI : <code class="bg-gray-800 px-2 py-1 rounded">wrangler d1 export</code> et 
                    <code class="bg-gray-800 px-2 py-1 rounded">wrangler r2 object get</code>
                </p>
            </div>
        </div>
    `;
...
async function extractPDFContent(file, progressCallback = null, batchMode = false) {
    ...
    try {
        ...
        // Intelligent sampling strategy: Beginning + End for better context
        // The last page often contains: conclusions, contacts, references, dates
        let pagesToExtract = [];  // Array of page numbers to extract
        let ocrPagesToExtract;
        
        // BATCH MODE: Optimized sampling - better quality/speed balance with light OCR
        if (batchMode) {
            if (totalPages <= 3) {
                // Very short docs: all pages
                pagesToExtract = Array.from({length: totalPages}, (_, i) => i + 1);
                ocrPagesToExtract = 1;  // Light OCR for scanned docs
            } else if (totalPages <= 10) {
                // Short docs: first 2 + middle + last
                const middle = Math.floor(totalPages / 2);
                pagesToExtract = [1, 2, middle, totalPages];
                ocrPagesToExtract = 1;  // OCR first page only
            } else if (totalPages <= 30) {
                // Medium docs: first 2 + middle + last
                const middle = Math.floor(totalPages / 2);
                pagesToExtract = [1, 2, middle, totalPages];
                ocrPagesToExtract = 1;  // OCR first page only
            } else if (totalPages <= 50) {
                // Long docs: first 2 + middle + last
                const middle = Math.floor(totalPages / 2);
                pagesToExtract = [1, 2, middle, totalPages];
                ocrPagesToExtract = 1;  // OCR first page only
            } else {
                // Very long docs: first 2 + two quarters + last
                const quarter1 = Math.floor(totalPages * 0.25);
                const quarter3 = Math.floor(totalPages * 0.75);
                pagesToExtract = [1, 2, quarter1, quarter3, totalPages];
                ocrPagesToExtract = 1;  // OCR first page only
            }
            DEBUG.info('PDF-EXTRACT', `⚡ BATCH MODE: pages [${pagesToExtract.join(', ')}] of ${totalPages}, OCR ${ocrPagesToExtract}`);
        } else if (totalPages <= 5) {
            // All pages for very short documents
            pagesToExtract = Array.from({length: totalPages}, (_, i) => i + 1);
            ocrPagesToExtract = Math.min(3, totalPages);
            DEBUG.info('PDF-EXTRACT', `Strategy: All ${totalPages} pages, OCR ${ocrPagesToExtract}`);
        } else if (totalPages <= 20) {
            // First 2 + last for short documents
            pagesToExtract = [1, 2, totalPages];
            ocrPagesToExtract = 2;
            DEBUG.info('PDF-EXTRACT', `Strategy: pages [1, 2, ${totalPages}], OCR 2`);
        } else if (totalPages <= 50) {
            // First 2 + last for medium documents
            pagesToExtract = [1, 2, totalPages];
            ocrPagesToExtract = 1;
            DEBUG.info('PDF-EXTRACT', `Strategy: pages [1, 2, ${totalPages}], OCR 1`);
        } else {
            // First + last for long documents
            pagesToExtract = [1, totalPages];
            ocrPagesToExtract = 1;
            DEBUG.info('PDF-EXTRACT', `Strategy: pages [1, ${totalPages}], OCR 1`);
        }
        
        if (progressCallback) {
            progressCallback(`📊 Analyse ${pagesToExtract.length} page(s) sur ${totalPages}...`, 5);
        }
        ...
        // Extract text from sampled pages (beginning + end)
        ...
        for (let index = 0; index < pagesToExtract.length; index++) {
            ...
            // Add context marker for last page
            const isLastPage = pageNum === totalPages;
            const pageLabel = isLastPage ? `[Page ${pageNum}/${totalPages} - DERNIÈRE PAGE]` : `[Page ${pageNum}/${totalPages}]`;
            fullText += `${pageLabel}\n${pageText}\n\n`;
            actualPagesExtracted++;
            
            const pageDuration = performance.now() - pageStartTime;
            DEBUG.debug('PDF-EXTRACT', `Page ${pageNum} text extracted in ${Math.round(pageDuration)}ms (${pageText.length} chars)`);
            
            if (progressCallback) {
                progressCallback(`Extraction page ${pageNum} (${index + 1}/${pagesToExtract.length})...`, 10 + ((index + 1) / pagesToExtract.length) * 30);
            }
            
            // In batch mode, stop early if we have enough text (but always extract last page if included)
            if (batchMode && fullText.length >= minTextThreshold && index < pagesToExtract.length - 1) {
                DEBUG.info('PDF-EXTRACT', `✅ Sufficient text found after ${actualPagesExtracted} pages (${fullText.length} chars)`);
                break;
            }
        }
        ...
        // Convert to base64 (High quality for better AI/OCR analysis)
        // Even in batch mode, quality is important for accurate text recognition
        const quality = batchMode ? 0.85 : 0.95;
        ...
        // Light OCR even in batch mode (1 page only for speed)
        if (isScanned && ocrPagesToExtract > 0) {
            ...
            try {
                // Run OCR on sampled pages (beginning + end strategy like text extraction)
                let ocrText = metaText;  // Keep metadata
                
                // Determine which pages to OCR (same strategy: beginning + end)
                let ocrPages = [];
                if (totalPages <= 3) {
                    ocrPages = Array.from({length: Math.min(ocrPagesToExtract, totalPages)}, (_, i) => i + 1);
                } else if (ocrPagesToExtract === 1) {
                    ocrPages = [1];
                } else if (ocrPagesToExtract === 2) {
                    ocrPages = [1, totalPages];
                } else {
                    // 3+ pages to OCR: first page + last page + one middle
                    ocrPages = [1, Math.ceil(totalPages / 2), totalPages];
                }
                
                for (let index = 0; index < ocrPages.length; index++) {
                    const pageNum = ocrPages[index];
                    DEBUG.info('PDF-EXTRACT-OCR', `Starting OCR on page ${pageNum} (${index + 1}/${ocrPages.length})`);
                    if (progressCallback) {
                        progressCallback(`🔍 OCR page ${pageNum} (${index + 1}/${ocrPages.length} sur ${totalPages} total)...`, 50 + ((index + 1) / ocrPages.length) * 40);
                    }
                        ...
                        'fra+eng', // French + English
                        {
                            logger: (m) => {
                                if (m.status === 'recognizing text' && progressCallback) {
                                    const progress = 50 + (index / ocrPages.length) * 40 + (m.progress * 40 / ocrPages.length);
                                    progressCallback(`🔍 OCR page ${pageNum} (${Math.round(m.progress * 100)}%)`, progress);
                                }
                            }
                        }
                    );
                    ...
                    // Add context marker for last page
                    const isLastPage = pageNum === totalPages;
                    const pageLabel = isLastPage ? `[Page ${pageNum}/${totalPages} - DERNIÈRE PAGE - OCR]` : `[Page ${pageNum}/${totalPages} - OCR]`;
                    ocrText += `${pageLabel}\n${text}\n\n`;
                }
                ...
                <form id="convert-modal-form" class="space-y-6">
                        ...
                        <div class="space-y-3">
                            <label class="flex items-start gap-3 bg-gray-600 p-3 rounded cursor-pointer hover:bg-gray-550 transition">
                                ...
                                <div class="flex-1">
                                    ...
                                    <!-- Sub-option: Skip first page -->
                                    <div id="modal-single-suboptions" class="mt-2 pl-6 border-l-2 border-blue-400">
                                        <label class="flex items-start gap-2 cursor-pointer">
                    ...
                    <div>
                        ...
                        <select 
                            ...
                            class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500"
                        >
                            <option value="0.7">Basse qualité - Fichier plus léger</option>
                            <option value="0.85">Qualité moyenne</option>
                            <option value="0.95" selected>Haute qualité (recommandé)</option>
                            <option value="1.0">Qualité maximale - Meilleure netteté</option>
                        </select>
                    </div>
...
async function reanalyzeDocument(token) {
    ...
    // Show AI analysis loading state in modal
    const aiZone = document.getElementById('ai-suggestions-zone');
    if (aiZone) {
        aiZone.classList.remove('hidden');
        aiZone.innerHTML = `
            <div class="bg-purple-900/30 border border-purple-600 rounded-lg p-4">
                ...
                <div class="bg-gray-800 rounded p-3">
                    <p id="ai-progress-message" class="text-sm text-gray-300">Initialisation...</p>
                    <div class="w-full bg-gray-700 rounded-full h-2 mt-2">
                        <div id="ai-progress-bar" class="bg-purple-500 h-2 rounded-full transition-all duration-300" style="width: 0%"></div>
                    </div>
                    <p id="ai-progress-percent" class="text-xs text-gray-400 mt-1">0%</p>
                </div>
                <p class="text-gray-400 text-xs mt-2">
                    ⏱️ Analyse en cours, veuillez patienter (environ 10-15 secondes)...
                </p>
            </div>
        `;
    }
    
    // Progress callback - update both button AND modal
    const updateProgress = (message, percent) => {
        button.innerHTML = `<i class="fas fa-spinner fa-spin mr-2"></i>${message}`;
    ...
    try {
        ...
        if (suggestions) {
            ...
            // Store suggestions in global scope for onclick handler
                ...
                aiZone.innerHTML = `
                    <div class="bg-green-900/30 border border-green-600 rounded-lg p-4">
                        <div class="flex items-center gap-2 mb-3">
                            <i class="fas fa-check-circle text-green-400 text-xl"></i>
                            <p class="text-white font-semibold">Analyse terminée !</p>
                        </div>
                        <p class="text-sm text-gray-300 mb-3">
                            L'IA a généré des suggestions pour ce document. Vous pouvez les appliquer automatiquement ou les modifier avant de sauvegarder.
                        </p>
                        <button 
                            onclick="applySuggestions(window._currentAISuggestions, 'edit')"
                ...
                aiZone.innerHTML = `
                    <div class="bg-red-900/30 border border-red-600 rounded-lg p-4">
                        <div class="flex items-center gap-2 mb-2">
                            <i class="fas fa-exclamation-circle text-red-400 text-xl"></i>
                            <p class="text-white font-semibold">Erreur d'analyse</p>
                        </div>
                        <p class="text-sm text-red-300">
                            L'IA n'a pas retourné de suggestions. Veuillez réessayer.
                        </p>
                    </div>
                `;
            }
        ...
        if (aiZone) {
            ...
            if (error.message.includes('API')) {
                errorDetails = `
                    <p class="text-sm text-red-300 mb-2">Erreur API : ${error.message}</p>
                    <div class="bg-gray-800 rounded p-3 text-xs text-gray-300">
                        <p class="font-semibold mb-1">💡 Vérifiez que :</p>
                        <ul class="list-disc list-inside space-y-1">
                            <li>L'IA est activée dans Sécurité > Configuration IA</li>
                            <li>La clé API Gemini est configurée</li>
                            <li>Vous avez des crédits API disponibles</li>
                        </ul>
                    </div>
                `;
            } else {
                errorDetails = `<p class="text-sm text-red-300">Détails : ${error.message}</p>`;
            }
            ...
            aiZone.innerHTML = `
                <div class="bg-red-900/30 border border-red-600 rounded-lg p-4">
                    <div class="flex items-center gap-2 mb-3">
                        <i class="fas fa-exclamation-circle text-red-400 text-xl"></i>
                        <p class="text-white font-semibold">❌ Erreur lors de l'analyse</p>
                    </div>
                    ${errorDetails}
                </div>
            `;
...
/**
 * Retry failed documents from batch analyze
 * Only processes documents that failed in the previous batch
 */
async function retryFailedDocuments() {
    DEBUG.group('🔄 RETRY FAILED DOCUMENTS');
    
    if (!window.failedDocuments || window.failedDocuments.length === 0) {
        DEBUG.warn('RETRY', 'No failed documents to retry');
        DEBUG.groupEnd();
        alert('Aucun document échoué à réessayer');
        return;
    }
    
    const retryCount = window.failedDocuments.length;
    DEBUG.info('RETRY', `Retrying ${retryCount} failed documents`);
    
    // Disable retry button and show loading
    const retryButton = document.getElementById('batch-retry-btn');
    if (retryButton) {
        retryButton.disabled = true;
        retryButton.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Réessai en cours...';
    }
    ...
    const errorDivs = detailsDiv.querySelectorAll('.bg-red-900\\/20, .bg-yellow-900\\/30');
    errorDivs.forEach(div => div.remove());
    
    statusDiv.textContent = `🔄 Réessai de ${retryCount} document(s)...`;
    progressBar.style.width = '0%';
    progressText.textContent = '0%';
    
    try {
        // Prepare documents for retry (already extracted)
        const documentsToAnalyze = window.failedDocuments.map(doc => ({
        ...
        DEBUG.info('RETRY', 'Sending to AI API', { 
            documentsCount: documentsToAnalyze.length
        });
        
        // Simulated progress (same as original batch analyze)
        ...
        DEBUG.info('RETRY', 'Retry result', result);
        
        if (result.success) {
            progressBar.style.width = '100%';
            progressText.textContent = '100%';
            statusDiv.textContent = `✅ Réessai terminé: ${result.analyzed} succès, ${result.failed} échecs`;
            ...
            // Add successful results to the display
            result.results.forEach((res, index) => {
                const resultId = `batch-retry-result-${index}`;
                        ...
                        <div class="flex items-start justify-between gap-3 mb-2">
                            <div class="flex-1">
                                <p class="text-white font-medium text-sm mb-1">
                                    <span class="text-green-400">✓ Réussi:</span> ${res.filename}
                                </p>
            ...
            if (result.errors.length > 0) {
                const all503Errors = result.errors.every(err => 
                    err.error && (err.error.includes('503') || err.error.includes('overloaded'))
                );
                    ...
                    detailsDiv.innerHTML += `
                        <div class="bg-yellow-900/30 border border-yellow-600 rounded-lg p-4 mb-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="text-white font-semibold mb-2">⚠️ Erreurs 503 persistantes</p>
                                    <p class="text-gray-300 text-sm mb-2">
                                        Les serveurs Gemini sont toujours surchargés. Attendez quelques minutes avant de réessayer.
                                    </p>
                                    <p class="text-green-300 text-sm mt-2">
                                        💡 Réessayez dans 5-10 minutes ou plus tard dans la journée.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            if (retryButton) {
                if (result.errors.length > 0) {
                    ...
                    retryButton.innerHTML = `<i class="fas fa-redo mr-2"></i>Réessayer les échecs (${result.errors.length})`;
                } else {
                    // All documents succeeded, hide retry button
            ...
            DEBUG.success('RETRY', `Retry completed: ${result.analyzed} success, ${result.failed} failed`);
            DEBUG.groupEnd();
        } else {
            DEBUG.error('RETRY', 'Retry failed', result.error);
            DEBUG.groupEnd();
            alert('Erreur lors du réessai: ' + result.error);
            
            if (retryButton) {
                retryButton.disabled = false;
                retryButton.innerHTML = `<i class="fas fa-redo mr-2"></i>Réessayer les échecs (${retryCount})`;
            }
        }
        
    } catch (error) {
        DEBUG.error('RETRY', 'Exception during retry', error);
        DEBUG.groupEnd();
        console.error('Retry error:', error);
        alert('❌ Erreur lors du réessai');
...
async function startBatchAnalyze() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Store failed documents globally for retry
            window.failedDocuments = result.errors.map(err => ({
                token: err.documentId,
                filename: err.filename,
                text: err.documentData?.text || '',
            ...
            // Check if all errors are 503 (API overload)
            const all503Errors = result.errors.every(err => 
                err.error && (err.error.includes('503') || err.error.includes('overloaded'))
            );
            
            // Display user-friendly notification for 503 errors
            if (result.errors.length > 0 && all503Errors) {
                detailsDiv.innerHTML += `
                    <div class="bg-yellow-900/30 border border-yellow-600 rounded-lg p-4 mb-4">
                        <div class="flex items-start gap-3">
                            <i class="fas fa-exclamation-triangle text-yellow-400 text-2xl"></i>
                            <div class="flex-1">
                                <p class="text-white font-semibold mb-2">⚠️ Analyse temporairement indisponible</p>
                                <p class="text-gray-300 text-sm mb-2">
                                    L'API Gemini est actuellement surchargée ou vous avez atteint une limite de quota.
                                </p>
                                <div class="bg-gray-800 rounded p-3 text-sm text-gray-300 mb-2">
                                    <p class="font-semibold text-white mb-1">Causes possibles:</p>
                                    <ul class="list-disc list-inside space-y-1">
                                        <li>Surcharge temporaire des serveurs Google (erreur 503)</li>
                                        <li>Limite de taux atteinte (trop de requêtes)</li>
                                    </ul>
                                </div>
                                <div class="bg-gray-800 rounded p-3 text-sm text-gray-300">
                                    <p class="font-semibold text-white mb-1">Que faire:</p>
                                    <ul class="list-disc list-inside space-y-1">
                                        <li>✅ Attendez 2-3 minutes</li>
                                        <li>✅ Cliquez sur "Réessayer les échecs" ci-dessous</li>
                                        <li>✅ Si le problème persiste, réessayez dans 1 heure</li>
                                    </ul>
                                </div>
                                <p class="text-green-300 text-sm mt-2">
                                    💡 <strong>Ne vous inquiétez pas</strong>: c'est normal et temporaire!
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            // Add "Réessayer les échecs", "Annuler" and "Fermer" buttons
            const buttonContainer = button.parentElement;
            
            // Show retry button only if there are failed documents
            const retryButtonHTML = result.errors.length > 0 ? `
                <button 
                    id="batch-retry-btn"
                    class="bg-orange-600 hover:bg-orange-700 text-white px-6 py-2 rounded transition font-semibold"
                    onclick="retryFailedDocuments()"
                >
                    <i class="fas fa-redo mr-2"></i>Réessayer les échecs (${result.errors.length})
                </button>
            ` : '';
            
            // Replace single button with multiple buttons
...
async function applyBatchSuggestions(resultId) {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Enhanced visual feedback: Green border and subtle background on entire card
            resultDiv.style.border = '2px solid #10B981';
            resultDiv.style.backgroundColor = 'rgba(16, 185, 129, 0.08)';
            resultDiv.style.transition = 'all 0.3s ease';
            
            // Debug: Verify button state after modification (FIXED: use closest instead of parentElement)
            DEBUG.info('APPLY-SUGGESTIONS', 'Button updated', {
                resultDivId: resultDiv.id,
                buttonParentId: button.closest('div[id]')?.id || 'UNKNOWN',
                hasGreenClass: button.classList.contains('bg-green-600'),
                classList: Array.from(button.classList)
            });
            
            // Show success notification toast
            showNotification(`✅ Métadonnées de "${suggestions.filename || data.filename}" appliquées avec succès !`, 'success');
            
            // Reload documents to show updated metadata in main list
            await loadDocuments();
            
            // Smart auto-close: Only close if ALL suggestions have been applied AND no retry button exists
            // Count remaining blue "Appliquer" buttons + check for orange "Retry" button
            setTimeout(() => {
                const remainingButtons = document.querySelectorAll('[id^="batch-result-"] button.bg-blue-600, [id^="batch-retry-result-"] button.bg-blue-600');
                const remainingCount = remainingButtons.length;
                
                // Check if "Réessayer les échecs" button exists (orange retry button)
                const retryButton = document.getElementById('batch-retry-btn');
                const hasRetryButton = retryButton && !retryButton.disabled;
                
                DEBUG.info('APPLY-SUGGESTIONS', `Remaining unapplied suggestions: ${remainingCount}, Has retry button: ${hasRetryButton}`);
                
                if (remainingCount === 0 && !hasRetryButton) {
                    // All suggestions applied AND no failed documents to retry - safe to auto-close
                    DEBUG.success('APPLY-SUGGESTIONS', 'All suggestions applied and no failures! Auto-closing modal...');
                    showNotification('🎉 Toutes les suggestions ont été appliquées ! Fermeture automatique...', 'success');
                    setTimeout(() => {
                        closeBatchAnalyze();
                    }, 1500);
                } else {
                    // Still have unapplied suggestions OR failed documents to retry - keep modal open
                    const reason = hasRetryButton ? 'failed documents to retry' : `${remainingCount} suggestion(s) remaining`;
                    DEBUG.info('APPLY-SUGGESTIONS', `Keeping modal open - ${reason}`);
                }
            }, 500);
...
// ==========================================
// DELETE ENTIRE LIBRARY (DANGEROUS OPERATION)
// ==========================================

/**
 * Show confirmation modal for deleting entire library
 * Requires password re-entry and typing confirmation phrase
 */
function showDeleteLibraryConfirmation() {
    DEBUG.group('🗑️ DELETE LIBRARY CONFIRMATION');
    DEBUG.warn('DELETE-LIBRARY', 'User initiated library deletion confirmation');
    
    const modal = document.createElement('div');
    modal.id = 'delete-library-modal';
    modal.className = 'fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4';
    modal.innerHTML = `
        <div class="bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full border-2 border-red-600 max-h-screen overflow-y-auto">
            <div class="bg-red-900 px-6 py-4 flex items-center justify-between border-b-2 border-red-600">
                <h2 class="text-2xl font-bold text-white flex items-center gap-3">
                    <i class="fas fa-exclamation-triangle text-yellow-400 text-3xl animate-pulse"></i>
                    Confirmation de Suppression
                </h2>
                <button onclick="closeDeleteLibraryModal()" class="text-white hover:text-gray-300 text-2xl">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            ...
            <div class="p-6 space-y-6">
                <!-- Critical Warning -->
                <div class="bg-red-900 bg-opacity-50 border-2 border-red-500 rounded-lg p-4">
                    <p class="text-red-200 font-bold text-lg mb-2">
                        ⚠️ ATTENTION : ACTION IRRÉVERSIBLE
                    </p>
                    <p class="text-red-300 text-sm">
                        Vous êtes sur le point de <strong>supprimer définitivement</strong> toute la bibliothèque.
                        Cette action ne peut <strong>PAS</strong> être annulée.
                    </p>
                </div>
                ...
                <div class="bg-gray-700 rounded-lg p-4">
                    <p class="text-white font-semibold mb-3 flex items-center gap-2">
                        <i class="fas fa-list-ul text-red-400"></i>
                    ...
                    <ul class="space-y-2 text-gray-300 text-sm">
                        <li class="flex items-start gap-2">
                            ...
                            <div>
                                <strong class="text-white">Base de données D1 :</strong>
                                Tous les enregistrements de la table <code class="bg-gray-800 px-2 py-1 rounded">documents</code>
                                (métadonnées, descriptions, tags, dossiers, statistiques)
                            </div>
                        </li>
                        <li class="flex items-start gap-2">
                            <i class="fas fa-server text-red-400 mt-1"></i>
                            <div>
                                <strong class="text-white">Bucket R2 :</strong>
                                Tous les fichiers PDF stockés dans le bucket <code class="bg-gray-800 px-2 py-1 rounded">storybook-pdfs</code>
                            </div>
                        </li>
                    </ul>
                </div>
                ...
                <div class="bg-yellow-900 bg-opacity-30 border border-yellow-700 rounded-lg p-4">
                    ...
                    <ul class="space-y-1 text-yellow-200 text-sm">
                        <li>❌ Aucun moyen de restaurer les données supprimées</li>
                        <li>❌ Les liens de partage existants cesseront de fonctionner</li>
                        <li>❌ Les QR codes générés deviendront invalides</li>
                        <li>❌ Toutes les statistiques de vues seront perdues</li>
                    </ul>
                </div>
                
                <!-- Security Form -->
                <form id="delete-library-form" class="space-y-4">
                    <!-- Password Re-entry -->
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-2">
                            <i class="fas fa-key mr-2 text-red-400"></i>
                            Confirmez votre mot de passe administrateur
                        </label>
                        <input 
                            type="password" 
                            id="delete-library-password" 
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-2">
                            ...
                            Tapez exactement la phrase suivante pour confirmer :
                        </label>
                        <div class="bg-gray-900 border border-gray-600 rounded-lg p-3 mb-2">
                            <code class="text-red-400 font-mono text-lg select-all">SUPPRIMER TOUTE LA BIBLIOTHEQUE</code>
                        </div>
                        <input 
                            type="text" 
                            id="delete-library-phrase" 
                            class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-red-500 focus:border-transparent font-mono"
                            placeholder="Tapez la phrase exacte (sensible à la casse)"
                            required
                            autocomplete="off"
                        />
                    </div>
                    ...
                    <!-- Final Checkbox -->
                    <label class="flex items-start gap-3 bg-gray-700 p-4 rounded-lg cursor-pointer">
                        <input type="checkbox" id="delete-library-checkbox" class="mt-1" required />
                        <div>
                            <p class="text-white font-semibold">Je comprends que cette action est irréversible</p>
                            <p class="text-gray-400 text-sm">
                                Je confirme avoir lu et compris les conséquences de cette suppression
                            </p>
                        </div>
                    </label>
                    
                    <!-- Error Message -->
                    <div id="delete-library-error" class="hidden bg-red-900 border border-red-700 rounded-lg p-3">
                        <p class="text-red-200 text-sm">
                            <i class="fas fa-exclamation-circle mr-2"></i>
                            <span id="delete-library-error-text"></span>
                        </p>
                    </div>
    ...
    document.getElementById('delete-library-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        await executeLibraryDeletion();
    });
...
function closeDeleteLibraryModal() {
    ...
    DEBUG.info('DELETE-LIBRARY', 'Modal closed');
}

/**
 * Execute the actual library deletion
 * Validates password and confirmation phrase before proceeding
 */
async function executeLibraryDeletion() {
    DEBUG.group('🗑️ EXECUTE LIBRARY DELETION');
    
    const password = document.getElementById('delete-library-password').value;
    const phrase = document.getElementById('delete-library-phrase').value;
    const checkbox = document.getElementById('delete-library-checkbox').checked;
    const errorDiv = document.getElementById('delete-library-error');
    const errorText = document.getElementById('delete-library-error-text');
    const submitBtn = document.getElementById('delete-library-submit-btn');
    
    // Validation
    if (!password) {
        errorDiv.classList.remove('hidden');
        errorText.textContent = 'Le mot de passe est requis';
        DEBUG.warn('DELETE-LIBRARY', 'Password missing');
        DEBUG.groupEnd();
        return;
    }
    
    if (phrase !== 'SUPPRIMER TOUTE LA BIBLIOTHEQUE') {
        errorDiv.classList.remove('hidden');
        errorText.textContent = 'La phrase de confirmation ne correspond pas (attention à la casse)';
        DEBUG.warn('DELETE-LIBRARY', 'Phrase mismatch', { provided: phrase });
        DEBUG.groupEnd();
        return;
    }
    ...
    if (!checkbox) {
        errorDiv.classList.remove('hidden');
        errorText.textContent = 'Vous devez cocher la case de confirmation';
        DEBUG.warn('DELETE-LIBRARY', 'Checkbox not checked');
        DEBUG.groupEnd();
        return;
    }
    ...
    submitBtn.disabled = true;
    submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Suppression en cours...';
    
    DEBUG.info('DELETE-LIBRARY', 'Validation passed, calling API');
    
    try {
        const response = await fetch('/api/admin/delete-library', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ password })
        });
        ...
        if (result.success) {
            DEBUG.success('DELETE-LIBRARY', 'Library deleted successfully', result);
            ...
            alert(`✅ Bibliothèque supprimée avec succès!\n\n` +
                  `📊 Statistiques :\n` +
                  `- ${result.deletedDocuments} documents supprimés de D1\n` +
                  `- ${result.deletedFiles} fichiers supprimés de R2\n` +
            ...
            DEBUG.error('DELETE-LIBRARY', 'Deletion failed', result.error);
            errorDiv.classList.remove('hidden');
            errorText.textContent = result.error || 'Erreur lors de la suppression';
            
            // Re-enable button
            submitBtn.disabled = false;
            submitBtn.innerHTML = '<i class="fas fa-trash-alt mr-2"></i>SUPPRIMER DÉFINITIVEMENT';
            
            DEBUG.groupEnd();
        }
    } catch (error) {
        DEBUG.error('DELETE-LIBRARY', 'Network error', error);
        errorDiv.classList.remove('hidden');
        errorText.textContent = 'Erreur réseau : ' + error.message;
